...
class sets extends \core\model
{
	...
	public function getsets($idpodkategori) {
		$data = $this->db->select("SELECT mt.id,ko.login as 'Login',id_podkategoria,mt.id_konto,ja.nazwa as 'Jezyk 1',jb.nazwa as 'Jezyk 2',pk.nazwa as 'Nazwa podkategorii',mt.nazwa as 'Nazwa zestawu',zestaw as 'Zawartosc zestawu',ilosc_slowek as 'Ilosc slowek',data_edycji as 'Data edycji' FROM zestaw mt LEFT JOIN konto ko on mt.id_konto = ko.id LEFT JOIN  jezyk ja on mt.id_jezyk1 = ja.id LEFT JOIN  jezyk jb on mt.id_jezyk2 = jb.id LEFT JOIN  podkategoria pk on mt.id_podkategoria = pk.id WHERE mt.id_podkategoria=".$idpodkategori);
		return $data;
	}

	public function getsetsByOne($id) {
		$data = $this->db->select("SELECT mt.id,ko.login as 'Login',id_jezyk1,id_jezyk2,mt.id_konto,id_podkategoria,ja.nazwa as 'Jezyk 1',jb.nazwa as 'Jezyk 2',pk.nazwa as 'Nazwa podkategorii',mt.nazwa as 'Nazwa zestawu',zestaw as 'Zawartosc zestawu',ilosc_slowek as 'Ilosc slowek',data_edycji as 'Data edycji' FROM zestaw mt LEFT JOIN konto ko on mt.id_konto = ko.id LEFT JOIN  jezyk ja on mt.id_jezyk1 = ja.id LEFT JOIN  jezyk jb on mt.id_jezyk2 = jb.id LEFT JOIN  podkategoria pk on mt.id_podkategoria = pk.id WHERE mt.id=".$id);
		return $data;
	}
